...
export class FileTreeProvider implements vscode.TreeDataProvider<FileItem> {
  ...
  async revert(filePath: string, fileType: string): Promise<void> {
    if (fileType === "M"){
      try {
        const result = await this.venvManager.executeDvcCommand(`dvc get . ${filePath} --rev HEAD --out ${filePath} -f`);
        if (result.stderr) {
      ...
      if (selection === 'Yes') {
          try {
            await fs.promises.unlink(path.join(this.workspaceRoot, filePath));
  ...
  private async displayDvcDiffCommand(pathName: string, changeType: string): Promise<void> {
      ...
      if (isImageFile(pathName)) {
        ...
        try {
          exec(`code -d ./${pathName} ${tempFilePath}`, { cwd: this.workspaceRoot }, (error, stdout, stderr) => {
            if (error) {
              vscode.window.showErrorMessage(`Error opening diff: ${stderr || error.message}`);
              return;
            }
          });
        } catch (err) {
          vscode.window.showErrorMessage("Error opening diff view.");
        }
      }
    };
    ...
    if (changeType === "A") {
      ...
      try {
        const result = await this.venvManager.executeDvcCommand(`dvc get . ${pathName} --rev HEAD --out ${tempFilePath} -f`);
        if (result.stderr) {
          vscode.window.showErrorMessage(`DVC Error: ${result.stderr}`);
          return;
        }
  ...
  private async runDvcDiffCommand(): Promise<void> {
    try {
      const result = await this.venvManager.executeDvcCommand('dvc diff HEAD --json');
      
      if (result.stderr && !result.stdout.trim()) {
        this.changedFiles = [{ path: "", name: result.stderr, type: ""}];
        return;
      }
      ...
      try {
        const diffData = JSON.parse(result.stdout); // Parse the JSON output
        this.changedFiles = [
